...
def main():
    ...
    parser.add_argument("-tl", type=int, required=False, default=16,
                        help="Number of processes used for preprocessing the low resolution data for the 3D low "
                             ...
                             "RAM")
    parser.add_argument("-tf", type=int, required=False, default=16,
    ...
    for t in tasks:
        ...
        # 什么时候将dataset.json 创建在cropped_out_dir中的。已经知道了
        # 从数据集配套的dataset.json文件中提取出数据类型，判断是不是CT类型，如果是CT类型会做 intensity propoerties 操作
